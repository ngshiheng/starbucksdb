...
docker-build:	## build datasette docker image.
	...
	datasette package $(SQLITE_FILE) --extra-options '--setting allow_download off --setting allow_csv_stream off --setting max_csv_mb 1 --setting default_cache_ttl 86400' --metadata data/metadata.json --install=datasette-cluster-map --install=datasette-hashed-urls --install=datasette-block-robots --install=datasette-vega --install=datasette-google-analytics  --tag $(IMAGE_NAME):$(TAG_DATE)
	datasette package $(SQLITE_FILE) --extra-options '--setting allow_download off --setting allow_csv_stream off --setting max_csv_mb 1 --setting default_cache_ttl 86400' --metadata data/metadata.json --install=datasette-cluster-map --install=datasette-hashed-urls --install=datasette-block-robots --install=datasette-vega --install=datasette-google-analytics  --tag $(IMAGE_NAME):latest
